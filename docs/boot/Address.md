...
| 0x9000          | -     | -                              | -                                    | จุดเริ่มต้นของ Stage 3 (`[ORG 0x9000]`) — โหลดโดย Stage 2  
| 0x9000          | 1     | `cli`                          | -                                    | ปิด interrupt  
| 0x9001          | 2     | `xor`                          | `ax, ax`                             | เคลียร์ AX  
| 0x9003–0x900D   | ~11   | `mov`                          | เซต DS=ES=SS=0, SP=0x8000            | เตรียม environment เริ่มต้น  
| 0x900E          | 5     | `call print_message`           | `si = stage3_msg`                    | แสดงข้อความ "Stage 3 - 64-bit Kernel Loader"  
| 0x9013          | ...   | `call check_a20`               | -                                    | ตรวจสอบ A20 (write 0x00/0xFF ที่ 0x0500/FFFF:0x0510)  
| 0x9020          | ...   | `call enable_a20`              | Fast A20 via port 0x92               | หากยังไม่เปิด: in al,0x92 / or al,2 / out 0x92,al  
| 0x9030          | ...   | `load_kernel_64:`              | -                                    | ตรวจสอบ INT 13h Extensions (AH=0x41, BX=0x55AA)  
| 0x9035–...      | ...   | `int 0x13` (AH=0x42 LBA)       | LBA 2048, 12x128 sectors → 0x10000   | โหลด x64 kernel (1536 sectors = 768 KB) ไปที่ 0x10000  
| 0x9080          | ...   | `setup_long_mode:`             | -                                    | สร้าง page tables ที่ 0x1000–0x3000 (clear 16 KB ก่อน)  
| 0x9090          | ...   | `mov cr4, eax`                 | CR4 |= PAE (bit 5)                   | เปิด PAE  
| 0x90A0          | ...   | `wrmsr`                        | MSR 0xC0000080 |= LME (bit 8)        | เปิด Long Mode ใน EFER  
| 0x90B0          | ...   | `lgdt [gdt_descriptor]`        | -                                    | โหลด 64-bit GDT (null/64-code/64-data/32-code/32-data)  
| 0x90C0          | 5     | `jmp 0x08:long_mode_start`     | Far Jump (CS=0x08)                   | flush pipeline เพื่อเข้า 64-bit mode  
| 0x90D0+         | -     | `[BITS 64] long_mode_start:`   | -                                    | set DS/ES/FS/GS/SS=0x10, RSP=0x7E00  
| 0x90E0+         | ...   | `mov rsi, 0x10000`             | src = kernel temp buffer             | เตรียม pointer ต้นทาง  
| 0x90F0+         | ...   | `mov rdi, 0x100000`            | dst = kernel final at 1 MB           | ชี้ไปยังตำแหน่งปลายทาง  
| 0x9100+         | ...   | `mov rcx, (1536*512)/8`        | = 98304 qwords (768 KB)              | จำนวน qword ที่ต้อง copy  
| 0x9110+         | ...   | `rep movsq`                    | copy 768 KB                          | คัดลอก x64 kernel ไปยัง 1 MB  
| 0x9120+         | ...   | `mov rax, 0x100000 / jmp rax`  | jump to kernel entry point           | กระโดดไปยัง kernel_main ที่ 0x100000  
| 0x9200+         | ~512  | `db`                           | ข้อความสำหรับ BIOS/VGA print           | เช่น "Stage 3", "A20 Error", "Copying kernel..."  
